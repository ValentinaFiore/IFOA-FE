...
function onlyLetters(stringa) {
  ...
  const newWord = stringa.replace(/\d/g, '');
  return newWord;
}

console.log(onlyLetters('C1IAO0ATU5TT1I'));
...
function isThisAnEmail(stri){
  ...
  return checkEmail.test(stri);
}


console.log(isThisAnEmail('valentina@gmail-com'));  
console.log(isThisAnEmail('vale@gmail.com'));
...
function rollTheDices(numberChoices) {
  let risultati = {
    sum: 0,
    values: []
  };
  ...
  for (var i = 0; i < numberChoices; i++) {
    let valoreDado = dice();
    risultati.sum += valoreDado;
    risultati.values.push(valoreDado);
  }

  return risultati;
}

let risultatoLanci = rollTheDices(3);
console.log("Risultato dei lanci:", risultatoLanci);
...
function isTodayMyBirthday(compleanno) {
  let oggi = new Date();
 let compleannoData = new Date(oggi.getFullYear(), compleanno.slice(0, 2) - 1, compleanno.slice(3, 5));

  return oggi.toDateString() === compleannoData.toDateString();
}

const compleanno = "2-12"; 
let oggiEIlMioCompleanno = isTodayMyBirthday(compleanno);
console.log("Oggi è il mio compleanno?", oggiEIlMioCompleanno);
...
let mioOggetto = { nome: "Giancarlo", età: 30, città: "Roma" };
let oggettoSenzaProprieta = deleteProp(mioOggetto, "età");

console.log("Oggetto originale:", mioOggetto);
console.log("Oggetto senza la proprietà 'età':", oggettoSenzaProprieta);
/* ESERCIZIO 12
...
console.log("Somma degli anni di produzione dei film:", sommaDegliAnni);
...
console.log("Film che contengono 'Lord' nel titolo:", risultatoRicerca);
/* ESERCIZIO 18
